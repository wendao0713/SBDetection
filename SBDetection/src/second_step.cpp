...
//===========================================================
vec pred(mat Y, mat phi, int p, int T, int k, int h) {
  mat concat_Y = zeros<mat>(k, T + h);
  concat_Y.cols(0, T - 1) = Y.cols(0, T - 1);
  for (int j = 1; j <= h; ++j) {
    mat temp = zeros<mat>(k, 1);
    for (int i = 1; i <= p; ++i) {
      temp += phi.cols((i - 1) * k, i * k - 1) * concat_Y.col(T + j - i - 1);
    }
    concat_Y.col(T + j -1) = temp;
  }
...
//===========================================================
// [[Rcpp::export]]
Rcpp::List backward_sel(mat data, vec lambda, vec pts, int p, int max_iteration = 1000, double tol = 1e-4, double step_size = 1e-3) {
  int k = data.n_cols;
  int T = data.n_rows;
  int m = pts.n_elem;
  
  vec L_n(m, fill::zeros);
  //double L_n_curr;
  
  // SHVAR FUN FULL PTS
  Rcpp::List try_result = break_var(data, lambda, pts, p, max_iteration, tol, step_size);
  double L_n_curr = try_result["L_n"];
  
  for (int mm = 0; mm < m; ++mm) {
    vec pts_temp = join_cols(pts.head(mm), pts.tail(m - mm - 1));
    // SHVAR FIT FUNCTION
    try_result = break_var(data, lambda, pts_temp, p, max_iteration, tol, step_size);
    L_n(mm) = try_result["L_n"];
  }
  return Rcpp::List::create(Rcpp::Named("L_n") = L_n,
                            Rcpp::Named("L_n_curr") = L_n_curr);
}

//===========================================================
// [[Rcpp::export]]
Rcpp::List second_step(mat data, vec lambda, vec pts, double omega, int p, int max_iteration = 1000, double tol = 1e-4, double step_size = 1e-3) {
  int m = pts.n_elem;
  if (m == 0) {
    return Rcpp::List::create(Rcpp::Named("pts") = pts,
                              Rcpp::Named("ic") = 0); 
  }
  int mm = 0;
  vec L_n;
  ...
  while (mm < m) {
    mm = mm + 1;
    Rcpp::List try_result = backward_sel(data, lambda, pts, p, max_iteration, tol, step_size);
    L_n = Rcpp::as<arma::vec>(try_result["L_n"]);
    L_n_curr = try_result["L_n_curr"];
    if (min(L_n) + (m - 1) * omega >= L_n_curr + m * omega) {
      ic = L_n_curr + (m - mm + 1) * omega;
      break;
    }
    if (min(L_n) + (m - 1) * omega < L_n_curr + m * omega) {
      // 移除 min(L.n) 对应的元素
      uvec indices_to_remove = find(L_n == min(L_n));
      pts.shed_rows(indices_to_remove);
      Rcpp::Rcout << "Updated pts: " << pts.t() << std::endl;
    }
  }
  return Rcpp::List::create(Rcpp::Named("pts") = pts,
                            Rcpp::Named("ic") = ic); 
